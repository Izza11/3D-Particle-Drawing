...
float particleSize = 3000 * sizeof(float);
float particleNum = 1000 * sizeof(float);

float time_sec;
float particles[3*1000];
float particleTime[1000];
float particlePlaced[1000];
float particleFade[1000];
...
GLuint partPlacedBuf[2];
GLuint partFadeBuf[2];

//Locations
GLint part_pos_loc;
GLint part_time_loc;
GLint part_placed_loc;
GLint part_fade_loc;

int drawBuf = 0;
ImVec4 color = ImColor(100, 150, 150);
...
void draw_gui()
{
	...
	ImGui::Checkbox("Draw cube", &cube_enabled);
	ImGui::Checkbox("Move Camera", &en_camera);
	ImGui::SliderFloat("View angle", &camangle, -1.0f, 1.0f);
	ImGui::SliderFloat3("Cam Pos", &campos[0], -1.0f, 1.0f);

	ImGui::ColorEdit3("Stroke Color", (float*)&color);
...
{	
	...
	int pcol_loc = glGetUniformLocation(skel_shader_program, "Pcolor");
	if (pcol_loc != -1) {
		glUniform3f(pcol_loc, color.x, color.y, color.z);
	}
...
void display()
{
	std::cout << en_camera << std::endl;
	if (en_camera) {
		M = glm::scale(glm::vec3(0.5f));
		V = glm::lookAt(campos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f))*glm::rotate(camangle, glm::vec3(0.0f, -1.0f, 0.0f));
		P = glm::perspective(80.0f, aspect, 0.1f, 100.0f); //not affine
	}
	else {
		M = glm::scale(glm::vec3(0.5f));
...
void initOpenGl()
{
	...
	cube_shader_program = InitShader2(cube_vs.c_str(), cube_fs.c_str());
	...
	for (unsigned int k = 0; k < 3000-3; k=k+3) {
		...
		particles[k] = randX;
		particles[k + 1] = -2000.0;
		particles[k + 2] = 0.0f;
		
	}
	
	for (float i = 0; i < 1000; i++) {
		particleTime[(int)i] = float(i/50);
		particlePlaced[(int)i] = 0.0f;
		particleFade[(int)i] = 1.0f;
	}
	...
	part_time_loc = glGetAttribLocation(skel_shader_program, "partTime");
	part_placed_loc = glGetAttribLocation(skel_shader_program, "partPlaced");
	part_fade_loc = glGetAttribLocation(skel_shader_program, "partFade");
	...
	glBindVertexArray(particlesVAO[0]);
		...
		glBufferData(GL_ARRAY_BUFFER, particleSize, nullptr, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_pos_loc);
		glVertexAttribPointer(part_pos_loc, 3, GL_FLOAT, GL_FALSE, 0, NULL);

		//partTimeBuf
		glGenBuffers(1, &partTimeBuf[0]);
		glBindBuffer(GL_ARRAY_BUFFER, partTimeBuf[0]);
		glBufferData(GL_ARRAY_BUFFER, particleNum, nullptr, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_time_loc);
		glVertexAttribPointer(part_time_loc, 1, GL_FLOAT, GL_FALSE, 0, NULL);

		//partPlacedBuf
		glGenBuffers(1, &partPlacedBuf[0]);
		glBindBuffer(GL_ARRAY_BUFFER, partPlacedBuf[0]);
		glBufferData(GL_ARRAY_BUFFER, particleNum, nullptr, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_placed_loc);
		glVertexAttribPointer(part_placed_loc, 1, GL_FLOAT, GL_FALSE, 0, NULL);

		//partFadeBuf
		glGenBuffers(1, &partFadeBuf[0]);
		glBindBuffer(GL_ARRAY_BUFFER, partFadeBuf[0]);
		glBufferData(GL_ARRAY_BUFFER, particleNum, nullptr, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_fade_loc);
		glVertexAttribPointer(part_fade_loc, 1, GL_FLOAT, GL_FALSE, 0, NULL);
	...
	glBindVertexArray(particlesVAO[1]);
		...
		glBufferData(GL_ARRAY_BUFFER, particleSize, particles, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_pos_loc);
		glVertexAttribPointer(part_pos_loc, 3, GL_FLOAT, GL_FALSE, 0, NULL);

		glGenBuffers(1, &partTimeBuf[1]);
		glBindBuffer(GL_ARRAY_BUFFER, partTimeBuf[1]);
		glBufferData(GL_ARRAY_BUFFER, particleNum, particleTime, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_time_loc);
		glVertexAttribPointer(part_time_loc, 1, GL_FLOAT, GL_FALSE, 0, NULL);

		glGenBuffers(1, &partPlacedBuf[1]);
		glBindBuffer(GL_ARRAY_BUFFER, partPlacedBuf[1]);
		glBufferData(GL_ARRAY_BUFFER, particleNum, particlePlaced, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_placed_loc);
		glVertexAttribPointer(part_placed_loc, 1, GL_FLOAT, GL_FALSE, 0, NULL);

		glGenBuffers(1, &partFadeBuf[1]);
		glBindBuffer(GL_ARRAY_BUFFER, partFadeBuf[1]);
		glBufferData(GL_ARRAY_BUFFER, particleNum, particleFade, GL_STREAM_DRAW);
		glEnableVertexAttribArray(part_fade_loc);
		glVertexAttribPointer(part_fade_loc, 1, GL_FLOAT, GL_FALSE, 0, NULL);
	...
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 1, partTimeBuf[0]);
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 2, partPlacedBuf[0]);
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 3, partFadeBuf[0]);

	// Transform feedback 1
	glBindTransformFeedback(GL_TRANSFORM_FEEDBACK, feedback[1]);
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 0, partPosBuf[1]);
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 1, partTimeBuf[1]);
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 2, partPlacedBuf[1]);
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 3, partFadeBuf[1]);
...
int main(int argc, char **argv)
{
	...
	int win = glutCreateWindow("Particle Drawing Demo - Izza Tariq");
